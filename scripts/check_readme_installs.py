...
def install_specs(readme: str) -> tuple[set[str], set[str], list[str]]:
    ...
    for match in INSTALL_RE.finditer(readme):
        ...
        if match.group("tool") == "pip":
            if "-e" in args or "--editable" in args or any(arg.startswith(".") for arg in args):
                local_commands.append(f"pip install {fragment}")
                continue
            candidates = [arg for arg in args if not arg.startswith("-")]
            if not candidates:
                raise ValueError(f"pip install command has no registry target: {match.group(0)!r}")
            pip_specs.update(candidates)
        else:
            candidates = [arg for arg in args if not arg.startswith("-")]
            if not candidates:
                raise ValueError(f"npm install command has no registry target: {match.group(0)!r}")
