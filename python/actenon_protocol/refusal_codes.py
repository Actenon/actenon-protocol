...
The catalogue ships as a pre-compiled JSON file
(``python/actenon_protocol/data/catalogue.v1.json``) so the runtime package
has **zero runtime dependencies**. The YAML source
(``refusals/catalogue.v1.yaml``) is the human-editable source of truth; the
JSON is regenerated by ``scripts/compile_yaml_to_json.py``.

If the ``yaml`` extra is installed (``pip install actenon-protocol[yaml]``)
and the JSON file is missing, the loader falls back to parsing the YAML
directly. This is a convenience for development; production deployments
should rely on the pre-compiled JSON.
"""
...
def _load_catalogue_from_json() -> dict | None:
    """Load the pre-compiled JSON catalogue.

    Returns None if the JSON file is not available.
    """
    ...
    try:
        ...
        pass
    # Fallback: resolve relative to the source repo (dev checkout).
...
def _load_catalogue_from_yaml() -> dict | None:
    """Load the YAML catalogue directly.

    Returns None if pyyaml is not installed or the YAML file is missing.
    ...
    try:
        ...
        return None
    # Try the packaged data directory first.
    try:
        with (
            resources.files("actenon_protocol.data")
        ...
        pass
    # Fallback: resolve relative to the source repo.
    repo_root = Path(__file__).resolve().parent.parent.parent
...
def _load_catalogue() -> dict:
    """Load the catalogue, preferring JSON (zero-dep) over YAML.

    Raises RuntimeError if neither can be loaded.
    """
    data = _load_catalogue_from_json()
    if data is not None:
        return data
    data = _load_catalogue_from_yaml()
    if data is not None:
        return data
    raise RuntimeError(
        "Could not load the refusal-code catalogue. The pre-compiled JSON "
        "(catalogue.v1.json) is missing, and pyyaml is not installed to "
        "parse the YAML source. Reinstall the package, run "
        "`python scripts/compile_yaml_to_json.py`, or install with "
